...
All notable changes to **ReverseOps** are documented in this file.
...
### Added — ReverseOps fork

- `panel/` — self-contained engagement dashboard (dependency-free SVG charts: severity/status donuts, findings timeline, category/case/ATT&CK bars; filterable findings table with PoC/evidence/remediation detail; works from `file://`)
- `scripts/mkreport.py` — findings aggregator: JSON findings + bilingual markdown reports → `panel/data/data.js` / `data.json`
...
- Routing: sigma vs malware, LLM jailbreak vs iOS jailbreak, full penetration/domain controller compromise vs AD domain controller, forensics vs OT ics; master-route.ps1 rewritten UTF-8 BOM for PS 5.1 CJK
...
First **formal** public release of the ReverseOps skill-router pack.
...
- Tag: `v1.0.0`
- Repository: https://github.com/xAmirHamza77/ReverseOps-Skill

[1.0.0]: https://github.com/xAmirHamza77/ReverseOps-Skill/releases/tag/v1.0.0
