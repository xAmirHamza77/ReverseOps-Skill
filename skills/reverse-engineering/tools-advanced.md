...
### d810-ng (IDA) — First choice for local processing

The modern maintained version of D-810 (Next Generation), integrating the **Z3 SMT** solver, covering multiple variants: OLLVM / Tigress / Hodur(PlugX) / Approov.
...
- Hacker's Delight bit-twiddling equivalences
- PlugX (Hodur) malware-specific MBA patterns

Installation: clone → copy into the IDA plugins directory → load with Ctrl-Shift-D
Source: https://github.com/w00tzenheimer/d810-ng
```

### obpo-plugin (IDA) — Strongest results, cloud plugin

Based on Hex-Rays **microcode** + data-flow tracking + program slicing + concolic execution. Recognized by the community as one of the most effective.
...
- Cloud plugin: target functions are uploaded to the obpo-server for processing (core is closed source, plugin is free)
- ⚠️ Use caution with sensitive samples (binaries are uploaded to a cloud service)
...
No IDA dependency; based on Miasm symbolic execution, BFS multi-layer processing, supports x86/x64 on Win/Linux.
...
Binary Ninja deflattening; the repo ships a hands-on case with the Android hardened sample `libvdog.so`. Source: amimo/ollvm-breaker
...
An Unicorn-based fallback for handling ARM64 .so files when IDA is unavailable. Source: GeT1t/deollvm
