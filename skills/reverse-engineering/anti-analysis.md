...
> For full OLLVM deobfuscation workflows, variant ecosystems (Hikari/Polaris/O-MVLL/Tigress/Hodur, etc.), and community tool surveys, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).
...
**Modern Variants (2026 Active Community):** Hikari (Anti Class Dump/String Encryption/Indirect Branch), Polaris (formerly Pluto, includes Trap Angr specifically targeting angr), O-MVLL (Python driver, common in Android hardening), Arkari (goron-based, indirect jumps can be countered via read-only data segments), amice (Rust, includes VM Flatten which requires VM reverse engineering rather than deflattener). For variant identification details, see Section 1 of ollvm-deobfuscation.md.

**Deobfuscation Tools (Sorted by Community Activity):**
- **obpo-plugin** (629⭐, IDA microcode+concolic cloud plugin, most effective): https://github.com/obpo-project/obpo-plugin
- **ollvm-breaker** (441⭐, Binary Ninja, Android .so real-world analysis): https://github.com/amimo/ollvm-breaker
- **ollvm-unflattener** (265⭐, Miasm symbolic execution, pure script x86/x64): https://github.com/cdong1012/ollvm-unflattener
- **d810-ng** (223⭐, IDA, integrated with Z3, covers OLLVM/Tigress/Hodur/Approov): https://github.com/w00tzenheimer/d810-ng — **Top local choice**
- **DeObfBR** (96⭐, specialized for BR indirect branch obfuscation): https://github.com/Mrack/DeObfBR
- **D-810** (Original version, mostly unmaintained, recommended to use d810-ng): pattern-based deobfuscation, MBA simplification
- **Miasm**: Symbolic execution for deobfuscation
- **Arybo** / **SiMBA**: MBA expression simplification

```bash
# d810-ng: Copy to IDA plugins directory, press Ctrl-Shift-D to load
# Select Unflattener + MBA simplification + Opaque predicate removal
# obpo: Right-click dispatcher → OBPO → Mark and process function (requires network connection)
# ⚠️ Pluto/Polaris Trap Angr pass causes angr to fail → Use d810-ng/Unicorn instead
```
