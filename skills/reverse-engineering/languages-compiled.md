...
.NET managed executables (C#-compiled exe/dll) have a complete independent workflow; see [`../dotnet-reverse/SKILL.md`](../dotnet-reverse/SKILL.md) for details.

**Quick Reference:**
- **Identification:** Non-zero CLR runtime header in PE header, `mscoree.dll` import, `mscorlib`/`System.Private.CoreLib` strings.
...
- **IL Priority:** Use an IL editor rather than a C# decompiler for critical logic analysis and patching (to avoid compiler-generated code recompilation distortion).
...
**Boundaries:** IL2CPP / NativeAOT compiled artifacts are native binaries (no CLR) and do not use dnSpy — use `ida-reverse/` / `radare2/` instead (see [seed-014](../field-journal/seed-014_unity-il2cpp-reverse.md)). For the Codegate 2013 two-phase XOR+AES-CBC pattern, see the `.NET Analysis` section in [tools.md](tools.md#net-analysis).
