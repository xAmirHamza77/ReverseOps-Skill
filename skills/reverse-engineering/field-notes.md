...
|---------|---------|------|
| `.sh` | ELF binary | Makes people think it is a script, lowering their guard |
| `.txt` | PE/ELF | Bypasses simple file-type filters |
| `.jpg`/`.png` | Executable or archive | Hidden inside an image |
| `.dll` | Actually a .NET assembly | Misdirects the analysis approach |
...
**"DriverLoader" does not necessarily load a driver; "Updater" does not necessarily update.**

Common name deception:

| Implied by file name | Actual behavior |
|-----------|---------|
...
- Ignore file-name implications and judge by actual code behavior
- Focus on syscalls such as `mmap`, `ptrace`, `/proc/self/mem`
- If you see "load a driver" but no `insmod`/`init_module` call, the name does not match the behavior
...
|------|-------------|
| Code has decryption/decompression logic | Set a breakpoint after decryption and dump the plaintext |
| Many indirect calls (function pointer tables) | Trace actual calls with strace/ltrace |
| Suspected anti-debugging | Check for ptrace calls with strace first |
| Embedded shellcode/payload | QEMU user-mode emulation |
| Unknown network protocol | Capture packets with tcpdump/Wireshark |

```bash
# strace syscall tracing (focus on these)
...
# Anti-debug check: see whether it self-traces with ptrace
strace ./binary 2>&1 | grep ptrace
# ptrace(PTRACE_TRACEME, ...) indicates anti-debugging
```

### Common Patterns in Process-Injection / Packed Samples

This class of samples (e.g. `LinYuDriverLoader`) typically:

1. **Does not actually load a kernel driver** (requires root, which is unavailable in most scenarios)
2. **Actually performs process injection**:
   - `ptrace` attaches to the target process
   - Reads/writes target memory via `/proc/<pid>/mem`
   - `mmap` maps shellcode into the target process space
3. **Embeds an encrypted payload**:
   - Decrypts a piece of shellcode at runtime
...
2. Check strings for obvious paths/library names/error messages
3. rabin2 -I for architecture/compiler/protections
4. Statically locate mmap/ptrace/open calls
5. If there is decryption logic → run dynamically past decryption and dump
