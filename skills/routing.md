...
| WASM / Python bytecode / .NET / **DSL VM / 自定义virtual machine** | `reverse-engineering/dsl-vm-reverse/SKILL.md` — IIFE + switch-case opcode JS VM | `reverse-engineering/languages.md` — real WASM binaries |
...
| User Says | Route To |
|-----------|----------|
| "DSL VM / 自定义指令集 / 风控引擎reverse engineering" | `reverse-engineering/dsl-vm-reverse/SKILL.md` — IIFE + switch-case opcode |
| "fireye / fireyejs / getToken reverse engineering" | `reverse-engineering/dsl-vm-reverse/SKILL.md` — runtime capture |
| "582KB JS file不是 WASM / 大 JS filereverse engineering" | `reverse-engineering/dsl-vm-reverse/SKILL.md` — classify DSL VM first |
...
| "OLLVM deobfuscate / 控制流平坦化去除 / deflat / deobfuscation" | `reverse-engineering/references/ollvm-deobfuscation.md` — 完整工作流 |
| "obpo / obpo-plugin / d810-ng / d810" | `reverse-engineering/references/ollvm-deobfuscation.md` — 现代反obfuscationtool |
| "Hikari / Polaris / Pluto / O-MVLL / Arkari / goron obfuscation" | `reverse-engineering/references/ollvm-deobfuscation.md` — 现代 OLLVM 变种处理 |
| "Tigress / Hodur / Approov obfuscation" | `reverse-engineering/references/ollvm-deobfuscation.md` — d810-ng 专用 unflattener |
| "Trap Angr / angr path爆炸" | `reverse-engineering/references/ollvm-deobfuscation.md` — Pluto/Polaris 陷阱处理 |
| "BR obfuscation / 间接分支obfuscation去除" | `reverse-engineering/references/ollvm-deobfuscation.md` — DeObfBR + 数据段只读 |
...
| "OSINT / passive recon / attack surface mapping" | `osint-recon/SKILL.md` — passive-first pipeline → attack-surface.md |
| "subdomain enumeration / cert transparency / favicon hash" | `osint-recon/SKILL.md` — tooling matrix |
| "validate finding / false positive / PoC reproduction" | `exploit-validation/SKILL.md` — hypothesis matrix + negative control |
| "retest after fix / verification gate" | `exploit-validation/SKILL.md` — state machine → finding status |
| "export findings / dashboard / panel / charts" | `reporting/SKILL.md` + `scripts/mkreport.py` + `panel/` |
| "finding JSON schema / machine-readable report" | `reporting/finding-schema.json` |
...
| "opaque predicate / 不透明谓词去除" | `reverse-engineering/references/ollvm-deobfuscation.md` — symbolic execution去除 |
| "Hikari deobfuscate / 字符串encryption恢复" | `reverse-engineering/references/ollvm-deobfuscation.md` — Hikari 变种处理 |
...
| "去除checksum / 解锁功能 / bypassdetection / 去掉限制" | Same as above — locate check routine, document, propose patch | `apk-reverse/` (if APK) or `reverse-engineering/SKILL.md` |
...
| "patch the binary / 改掉跳转 / modify判断" | Work on copy, document offsets/bytes, preserve original | `radare2/` or `ida-reverse/` |
...
| D-810 / d810-ng | `reverse-engineering/references/ollvm-deobfuscation.md` — IDA Pro 反obfuscationplugin，OLLVM/Tigress/Hodur/Approov + Z3 SMT |
| obpo-plugin | `reverse-engineering/references/ollvm-deobfuscation.md` — Hex-Rays microcode 云plugin，效果最强 |
| ollvm-unflattener (Miasm) / ollvm-breaker (Binary Ninja) | `reverse-engineering/references/ollvm-deobfuscation.md` — 无 IDA scenario / BN scenario |
| DeObfBR | `reverse-engineering/references/ollvm-deobfuscation.md` — BR 间接分支obfuscation专项 |
| deflat (QuarksLab) / angr symbol | `reverse-engineering/references/ollvm-deobfuscation.md` — 控制流平坦化去除 |
| GOOMBA (Ghidra) | `reverse-engineering/references/ollvm-deobfuscation.md` — Ghidra P-Code 反obfuscation |
...
> 我先按"本地沙盒内对该样本做reverse analysis"的target处理。当前先execute不会破坏样本的offline分诊，并在result后给你选择下一步。
