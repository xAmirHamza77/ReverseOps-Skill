...
| Python (render script) | ✓ | Already included in bootstrap | `scripts/render_diagram.py` dependency |

### Notes

This skill primarily outputs text-based diagram source (Mermaid/DOT/PlantUML) and does not necessarily require local rendering tools. The corresponding renderer is needed only when the user explicitly asks for PNG/SVG/PDF files.

If a renderer is unavailable, `scripts/render_diagram.py` outputs installation hints instead of failing with an error.
...
**Trigger conditions**: user says "draw a diagram", "flowchart", "architecture diagram", "attack path diagram", "sequence diagram", "Mermaid", "Graphviz", "PlantUML"
**Downstream outputs**:
- Generated diagrams can be embedded in reports from `docs-generator/`
- Attack path diagrams can be used alongside penetration test reports from `pentest-tools/`

**Peer modules**: `docs-generator/` (diagrams embedded in reports)
