...
| output字段 | 影响 | 应对 |
|---------|------|------|
| `NX disabled` | 栈可execute | 直接塞 shellcode |
| `Canary found` | 栈溢出会被detection | must先 leak canary 或bypass (forked process / 格式化字符串) |
| `PIE enabled` | .text 基址随机 | must leak 一个代码地址 |
| `No PIE` | .text 固定 | gadget 地址写死 |
| `Full RELRO` | got 不可写 | 不能改 got，走 ret2libc / one_gadget |
| `Partial RELRO` | got 可写 | 可以改 got 表 |
| `FORTIFY` | 部分 libc 函数被替换为 `_chk` version | `read_chk` 仍然能溢出，`strcpy_chk` 不行 |
...
| 架构 | parameter传递 | 返回 | 备注 |
|------|---------|------|------|
| x86 (32-bit) | 栈传参（cdecl: 调用者清栈） | eax | 栈结构：ret_addr, arg1, arg2, ... |
| x86-64 SysV | rdi, rsi, rdx, rcx, r8, r9, 栈 | rax | rsp must 16-byte 对齐到 call entry point |
| ARM32 | r0-r3, 栈 | r0 | lr 保存返回地址，bx lr 返回 |
| ARM64 | x0-x7, 栈 | x0 | 类似 SysV，更严格的对齐 |

## ret2libc 完整 pwntools template
...
# automatic patchelf 让本地用题目给的 libc
...
payload  = b'A' * OFFSET
payload += p64(ret)        # critical：provision 16-byte 对齐
payload += p64(pop_rdi)
payload += p64(binsh)
payload += p64(system)
...
原因：libc 的 system → do_system → 内部某处 movaps xmm0, [rsp]
       要求 rsp 16-byte 对齐
failure：你的 ROP 链跳进 system 时，rsp 末位是 0x8 而不是 0x0
remediation：在 ROP 链里插一个 `ret` gadget（消耗 8 字节，让 rsp 重新对齐）
```

## ret2csu（万能 gadget）

当二进制里没有 `pop rdx; ret` 这种第三parameter gadget 时，用 `__libc_csu_init` 里的固定结构（glibc < 2.34 静态链接的程序里都有）。
...
def csu(rdi, rsi, rdx, call_target):
    ...
    p += p64(call_target)    # r12 = [r12+rbx*8] 解引用得到target
    p += p64(rdi)            # r13
    p += p64(rsi)            # r14
    p += p64(rdx)            # r15
    p += p64(csu_call)
...
适用：bss 里写一个函数指针，再用 csu 调用它，常用于 `read(0, bss, 0x100)` phase后跳到 bss execute ROP。
...
**坑**：one_gadget 在某些 libc version（2.34+）约束极难满足，老老实实 ret2libc 更稳。

## libc-database 反查

scenario：题目没给 libc，只能 leak 几个函数地址反推version。
...
| 问题 | 现象 | 解决 |
|------|------|------|
| libc version错 | 本地通，远程 SIGSEGV in system | leak 后用 libc-database 反查实际version |
| 栈对齐 | system 立刻段error | 加一个 `ret` gadget |
| network延迟 | recv 收到一半 | 用 `recvuntil(b'锚字符串')`，不用 `sleep` |
| 缓冲 | sendline 发完没反应 | 改 `sendlineafter`，明确等到 prompt 再发 |
| ASLR 浮动 | 概率success | 看是否 byte-level brute（1/16 概率不算稳定） |
...
context                # automatic显示 reg + stack + code（default开）
...
- **canary 在 fork 子process里不变** — forking server 可以一次 byte 一次 byte brute force（1/256 × 7 字节）
