...
heap利用的所有技术都跟 glibc version强绑定。先确认version：
...
| 2.35+ | 进一步加固 | 同 2.34，FILE path仍available |
...
# 申请大于 tcache scope的 chunk（>0x408），free 进 unsorted bin，残留 main_arena 指针
for i in range(8):
    add(i, 0x80)
add(8, 0x80)  # 防止合并
for i in range(7):
...
```text
原理：tcache/fastbin 的 fd write时被 PROTECT_PTR 异或：
    PROTECT_PTR(pos, ptr) = (pos >> 12) ^ ptr

bypass：
1. must先 leak 一个heap地址（heap base）
...
```text
critical点：
1. fastbin 单链表（只有 fd），无 size 检查除了 chunk size mustmatch
2. 2.27 后 tcache priority，fastbin 只有 tcache 满了才用
3. 仍然需要伪造一个看起来像 chunk 的memory（size 字段 = 真实 chunk size，± 一些）
```
...
# 把 fd 改成 fake chunk（要求 fake_addr + 8 处的 size 字节match 0x70）
...
2.32 起也加了 chunk size 检查，但仍available于改 global_max_fast、_IO_list_all 等
...
| House of Lore | 全version | 伪造 small bin 链 → 返回任意地址 |
...
| House of Einherjar | 全version | 伪造 prev_size + PREV_INUSE=0 → backward consolidation |
| House of Botcake | 2.27+ | tcache + unsorted bin 组合，bypass tcache double free 检查 |
| House of Husk | 2.27+ | 改 printf 的 hook table（__printf_function_table） |
| House of Cat | 2.34+ | _IO_wfile_seekoff vtable 利用，针对无 hook version |
...
## libc 2.34+ 没有 hook 之后的替代path

### FILE struct 攻击（_IO_2_1_stdout_ / _IO_2_1_stderr_）

```text
target：当程序调用 puts/printf 时，最终走到 _IO_file_xsputn → _IO_OVERFLOW → 调用 vtable
劫持：
...
```text
__call_tls_dtors 遍历，结构类似，同样要bypass PTR_DEMANGLE
适用：程序退出时会走，比 FILE 攻击更通用
```

## pwndbg / GEF heapdebuggingcommand
...
tcache            # 单独看 tcache
find_fake_fast <addr> <size>  # 找一个能用作 fake chunk 的 fd write点
vis_heap_chunks   # 可视化heap布局
...
## 典型 pwntools template（heap menu 题）
...
# === 后续根据vulnerability类型选择技术栈 ===
```

## Notes

- **glibc version是第一性问题** — 同一个二进制配 2.27 libc 和 2.34 libc，利用path完全不同
...
- **远程heap喷不稳** — 服务端 fork 模型每次连接 brk/mmap 可能不同，要做随机化Testing
- **不要在 attack 链中遗留 unsorted 残留** — main_arena 指针出现在意外的 chunk 里会让后续 show output错乱
- **safe-linking error率** — 计算 PROTECT_PTR 时记得是 `pos >> 12`，pos 是要write的地址不是要指向的地址
