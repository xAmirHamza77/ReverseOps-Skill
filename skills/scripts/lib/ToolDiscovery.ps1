...
function Get-ReverseToolCatalog {
    ...
    return @(
        ...
        [pscustomobject]@{
            Name = 'java'
            Skill = 'apk-reverse'
            Purpose = 'run jar 与 Java Toolchain'
            VersionArgs = @('-version')
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'node'
            Skill = 'js-reverse'
            Purpose = 'run Node 侧 JS 复现与 MCP 客户端'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'npx'
            Skill = 'js-reverse'
            Purpose = 'run临时 npm 包与 MCP entry point'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'jshookmcp'
            Skill = 'js-reverse'
            Purpose = '通过 npx start @jshookmcp/jshook MCP（仍需先configuration并启用 MCP server）'
        ...
        [pscustomobject]@{
            Name = 'reqable-mcp'
            Skill = 'pentest-tools'
            Purpose = '通过 npx start Reqable 桌面客户端 MCP（仍需先installation并start Reqable）'
        ...
        [pscustomobject]@{
            ...
            Purpose = '浏览器automatic化（Playwright）：打开页面、点击、填表、爬取、截图'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            ...
            Skill = 'reverse-engineering'
            Purpose = 'Ghidra 无头分析（free IDA 替代）'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'jeb-pro'
            Skill = 'apk-reverse'
            Purpose = 'JEB Pro commercial Android / ARM decompilation器（需用户自备有效License）'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'pentestswarm'
            Skill = 'pentest-tools'
            Purpose = '群体智能自主penetration与 MCP execute'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
...
function Get-ReverseBootstrapManifestPath {
    ...
    return Join-Path (Get-ReverseOpsRoot) 'scripts\bootstrap-manifest.json'
}
