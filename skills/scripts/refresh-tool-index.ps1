...
# Skills not part of ReverseOps core (local-only / extracted / never index)
$bannedSkillExact = @(
    'blockchain-security',
    'bitcoin-puzzle',
    'web3-skill',
...
$markdownLines = @(
    '# reverse engineeringtoolindex',
    '',
    "- 扫描时间: $generatedAt",
    '- routingentry point: `SKILL.md` → `routing.md` → 对应子 skill',
    '- description: 本表由 `skills/scripts/refresh-tool-index.ps1` automaticgenerate，priority用于 Claude routing和toolpath确认。',
    '- 注意: 对于 jshookmcp 这类 MCP server，`yes` 只表示本机具备通过 node/npx 拉起它的条件，不表示它已经在 MCP configuration里注册并启用。',
    '',
...
$markdownCapLines = @(
    ...
    '## Capability Status View',
    '',
    '| capability | toolavailable | Ready | MCP registered | 服务online | MCP HTTP | 可auto-install | installation方式 |',
    '|------|---------|-------|-----------|---------|-----------|---------|'
)
...
$markdownCapLines += ''
$markdownCapLines += '> ✓ = 是 | ✗ = 否 | — = 不适用或未detection'
$markdownCapLines += ''
