...
# ReverseOps routing + ops contract gates (skill-router only; no host platform runtime)
...
foreach ($rp in @($rulesEn, $rulesZh)) {
    ...
    if ($rt -match 'auth\.status\s*=\s*granted|auth.status=granted|未就绪forbidden|MUST NOT ACT against targets|forbidden对target ACT') {
...
if ($id -match 'tool-index|bootstrap|field-journal|routing') { Ok 'identity keeps ReverseOps DNA' } else { Bad 'identity missing DNA' }
