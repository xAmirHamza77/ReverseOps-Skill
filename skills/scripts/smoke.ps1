...
# ReverseOps smoke entrypoint: verify + script parse + master-route sample matrix.
# Usage:
...
Write-Host ("=== ReverseOps smoke | LogDir={0} ===" -f $LogDir)
...
$cases = @(
    ...
    @{ Name = 'zh-apk'; Hint = '安卓 APK 加固 decompilation'; Expect = 'apk-reverse' },
    @{ Name = 'zh-pentest'; Hint = 'penetration testing port scanning SQLinjection'; Expect = 'pentest-tools' },
    @{ Name = 'zh-js'; Hint = '前端signature JSreverse engineering encryptionparameter'; Expect = 'js-reverse' }
)
$routeOk = 0
$routeFail = 0
