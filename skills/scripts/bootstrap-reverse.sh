...
TOOLS_ROOT="${REVERSEOPS_TOOLS_DIR:-$HOME/tools}"
if [[ "$TOOLS_ROOT" != /* ]]; then
  TOOLS_ROOT="$PWD/$TOOLS_ROOT"
fi
if [[ -z "$TOOLS_ROOT" || "$TOOLS_ROOT" == "/" || "$TOOLS_ROOT" == "$HOME" ]]; then
  echo "Unsafe REVERSEOPS_TOOLS_DIR: $TOOLS_ROOT" >&2
  exit 2
fi
...
Notes:
  ...
  - Override install root with REVERSEOPS_TOOLS_DIR=~/tools.
EOF
}
...
req = urllib.request.Request(url, headers={'User-Agent':'ReverseOps-bootstrap'})
