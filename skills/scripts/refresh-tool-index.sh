...
# Parity with skills/scripts/refresh-tool-index.ps1 (the "capabilitystatus视图" block).
...
PY
  ...
  # Append the markdown capability table (mirrors the headings used in the ps1 version)
  {
    ...
    echo ""
    echo "| capability | toolavailable | Ready | MCP registered | 服务online | MCP HTTP | 可auto-install | installation方式 |"
    echo "|------|---------|-------|-----------|---------|----------|-----------|---------|"
  } >> "$OUTPUT_MD"
