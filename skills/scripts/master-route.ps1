...
# ReverseOps PRIMARY router. EN + ZH keyword patterns; UTF-8 BOM source for Windows PowerShell 5.1.
param(
    [string] $Hint = '',
    [string] $OutDir = ''
)
...
if ($t -match '\bapk\b|smali|jadx|apktool|android.?reverse|安卓|decompilation.?apk|apk.?加固|重打包') { [void]$sel.Add('R1') }
# iOS mobile: do NOT bare-match 越狱 when LLM/model context (R14)
if ($t -match '\bipa\b|ios.?reverse|objection|mobsf|mobile.?reverse|ios.?reverse engineering') { [void]$sel.Add('R2') }
if ($t -match '越狱' -and $t -notmatch '模型|hint词|llm|prompt|jailbreak|garak|red team.?ai|ai.?red team') { [void]$sel.Add('R2') }
if ($t -match 'jailbreak' -and $t -match 'ios|iphone|ipad|mobile|objection|ipa') { [void]$sel.Add('R2') }
if ($t -match 'js.?reverse|webpack|cryptojs|frontend.?sign|jshook|cdp|encrypted.?param|前端.?signature|js.?reverse engineering|encryption.?parameter|webpack.?reverse engineering') { [void]$sel.Add('R3') }
if ($t -match 'dsl.?vm|fireye|opcode.?vm|custom.?vm|自定义.?virtual machine') { [void]$sel.Add('R4') }
if ($t -match '\.net|dnspy|de4dot|confuserex|csharp|dotnet|c#') { [void]$sel.Add('R5') }
if ($t -match '\bida\b|decompile|disassembl|decompilation|disassembly|静态.?分析.?二进制') { [void]$sel.Add('R6') }
if ($t -match 'radare|\br2\b') { [void]$sel.Add('R7') }
if ($t -match 'firmware|binwalk|iot|emba|firmadyne|firmware|routing器.?firmware|嵌入式') { [void]$sel.Add('R8') }
...
if ($t -match 'attack.?chain|red.?team|lateral|domain.?pentest|internal.?network|full.?pentest|完整.?penetration|从外网|打到域控|red team|横向.?移动|内网.?penetration') { [void]$sel.Add('R10') }
if ($t -match 'nmap|nuclei|sqlmap|ffuf|pentest|src.?hunt|bug.?bounty|waf.?bypass|penetration.?Testing|port.?扫描|vulnerability.?扫描|directory.?brute force|sql.?injection|众测') { [void]$sel.Add('R11') }
if ($t -match 'graphql|bola|bfla|api.?secur|interface.?安全|privilege escalation|unauthorized.?访问|rest.?api.?secur') { [void]$sel.Add('R12') }
if ($t -match '\boauth\b' -and $t -notmatch 'oauth2|oidc|saml|sso|openid|联邦|单点') { [void]$sel.Add('R12') }
if ($t -match 'sbom|supply.?chain|trivy|gitleaks|syft|supply chain|dependency.?扫描') { [void]$sel.Add('R13') }
if ($t -match 'llm|prompt.?inject|jailbreak|agent.?secur|garak|owasp.?llm|hint词.?injection|模型.?red team|模型.?越狱|llm.?越狱|hint词.?越狱|ai.?red team') { [void]$sel.Add('R14') }
if ($t -match 'bindiff|symbol.?migrat|pdb|符号.?迁移|version.?对比') { [void]$sel.Add('R15') }
if ($t -match 'n-?day|patch.?diff|patch.?tuesday|patch.?差分|patch.?分析') { [void]$sel.Add('R16') }
if ($t -match '\bpwn\b|rop|ret2libc|heap.?overflow|stack.?overflow|pwntools|栈溢出|heap溢出') { [void]$sel.Add('R17') }
if ($t -match 'edr|av.?bypass|syscall|amsi|etw.?patch|hell.?s.?gate|免杀|反病毒') { [void]$sel.Add('R18') }
if ($t -match 'playwright|browser.?auto|desktop.?auto|openreverse|fill.?form|浏览器.?automatic化|桌面.?automatic化|automatic.?填表') { [void]$sel.Add('R19') }
if ($t -match 'writeup|write.?report|generate.?report|\breport\b|写.?report|出.?report|penetration.?report|reverse engineering.?report') { [void]$sel.Add('R20') }
if ($t -match 'diagram|mermaid|graphviz|plantuml|flowchart|process图|架构图|时序图|status图|数据流图|attack path图|er.?图|画图|图表') { [void]$sel.Add('R39') }
if ($t -match 'protocol.?reverse|custom.?protocol|protobuf|grpc|pcap.?protocol|wireshark.?dissector|protocol.?reverse engineering|自定义.?protocol|流量.?reverse engineering') { [void]$sel.Add('R21') }
if ($t -match 'ghidra|ghidra.?mcp|analyzeheadless|无.?ida|open source.?decompilation') { [void]$sel.Add('R22') }
if ($t -match 'kubernetes|\bk8s\b|container.?escape|docker.?escape|kube-?bench|cloud.?secur|imds|169\.254\.169\.254|container.?逃逸|云.?安全|k8s.?penetration') { [void]$sel.Add('R23') }
if ($t -match 'active.?directory|\bad\b.?cs|bloodhound|kerberoast|as-?rep|certipy|ntlm.?relay|dc.?sync|kerberos.?攻击|ad.?certificate') { [void]$sel.Add('R24') }
if ($t -match '域.?penetration|域控' -and $t -notmatch '完整.?penetration|从外网|打到域控|attack.?chain|full.?pentest') { [void]$sel.Add('R24') }
if ($t -match 'forensic|volatility|memory.?dump|plaso|timeline.?explorer|forensics|memory.?转储|应急.?response.?forensics') { [void]$sel.Add('R25') }
if ($t -match 'code.?audit|sast|semgrep|codeql|source.?review|白盒|代码.?审计|静态.?应用.?安全') { [void]$sel.Add('R26') }
if ($t -match 'threat.?hunt|detection.?engineer|blue.?team|sigma.?rule|\bsigma\b|威胁.?狩猎|detection.?工程|blue team.?狩猎|detection.?rule') { [void]$sel.Add('R27') }
...
if ($t -match 'wifi|wi-?fi|aircrack|airmon|wpa.?handshake|wireless.?pentest|无线.?penetration|wifi.?攻击') { [void]$sel.Add('R29') }
if ($t -match 'browser.?extension|chrome.?extension|\bcrx\b|\bxpi\b|mv3.?extension|浏览器.?extension|chrome.?extension') { [void]$sel.Add('R30') }
if ($t -match 'macos|mach-?o|codesign|objective-?c|swift.?reverse|xpc|mac.?reverse engineering|苹果.?桌面') { [void]$sel.Add('R31') }
if ($t -match 'thick.?client|desktop.?client|electron.?app|winforms|wpf|厚客户端|桌面.?客户端') { [void]$sel.Add('R32') }
if ($t -match '\bgolang\b|\brustc\b|go.?binary|stripped.?go|gore.?sym|go.?malware|rust.?binary|go.?reverse engineering|rust.?reverse engineering') { [void]$sel.Add('R33') }
if ($t -match 'uart|jtag|swd|debug.?pad|flashrom|hardware.?debugging|串口.?shell|芯片.?提取') { [void]$sel.Add('R34') }
if ($t -match 'database.?secur|\bmysql\b|\bpostgres|mongodb|redis.?secur|mssql|数据库.?安全|数据库.?penetration') { [void]$sel.Add('R35') }
if ($t -match 'phish|spf|dkim|dmarc|bec\b|email.?secur|phishing.?邮件|邮件.?安全') { [void]$sel.Add('R36') }
...
if ($t -match 'ollvm|anti-?debug|unicorn|angr|gdb|deobfuscat|控制流平坦|反debugging') { [void]$sel.Add('R0') }
if ($t -match 'frida' -and $t -notmatch '\bapk\b|smali|jadx|android|安卓') { [void]$sel.Add('R0') }
if ($t -match 'reverse|reverse engineering' -and $t -notmatch 'apk|js.?reverse|ios|mobile|\.net|firmware|malware|安卓|firmware|恶意|protocol|ghidra|extension|macos|mach|golang|rust|工控|厚客户端|forensics|protocol|extension') { [void]$sel.Add('R0') }
...
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    ...
    if ($packageRoot -and (Test-Path -LiteralPath $packageRoot)) {
        ...
        $OutDir = Join-Path $tmpBase ("ReverseOps-route\master-route-{0}" -f $stamp)
    }
}
...
[void]$sb.AppendLine('# ReverseOps Master route (PRIMARY)')
[void]$sb.AppendLine(("- created: {0}" -f (Get-Date -Format 'o')))
[void]$sb.AppendLine(("- package: ReverseOps"))
